...
subtitle: Lesson 3 - Controlling the search and assessing uncertainty <br> [Click here to view presentation online](https://qfcatmsu.github.io/MLE-Software/lesson3/MLE_lec3.html){style="position:absolute;top:40%lesson1;left:35%;font-size:20px;"}
author: Jim Bence
...
-   Code squib
    ...
    #fit with bounds, here ... are the other arguments we usually use
...
-   Modify the vonB program to bound the parameters (bounds in slide on bounds
    reasonable choices)

-   Modify the vonB program to not estimate log_linf using a map

-   See if you can combine setting bounds and fixing a parameter
...
$$
L R T=-2 \log \frac{\sup \left(L(\underline{\theta}), \underline{\theta} \in \underline{\theta_{0}}\right)}{\sup (L(\underline{\theta}), \underline{\theta} \in \underline{\theta})}=-2\left[\log \left(\sup \left(L(\underline{\theta}), \underline{\theta} \in \underline{\Theta}_{0}\right)\right)-\log (\sup (L(\underline{\theta}), \underline{\theta} \in \underline{\theta})]\right.
$$
...
-   I will demo doing this for one data set and have you attempt to do a full
    simulation study.
