 ...
 # This version has a bug - don't use except as debugging exercise!

library(RTMB);

mp_df<-read.csv(file="more_examples/data/newmulti.csv");
head(mp_df);
...
f = function(pars){
 ...
 #Sigma=cm*(vb_sd%o%vb_sd); gives same result as above
 vonb_pars<-matrix(nrow=nponds,ncol=3)
 for(i in 1:nponds) {
   vonb_pars[i,] =c(log_linf[i],log_k[i],log_l2[i]);
 }
