...
-   multinomial apps often use data as proportions and "effective sample size",
    which often differs from actual sample size

-   In applications the negative log likelihood was had written in terms of
    proportions and ESS

-   You can emulate this in RTMB by multiplying the proportions by the ESS and
    providing these as the data to dmultinom (don't set size!)

    -   The NLL returned will depend on the ESS but ESS cannot be estimated
        (nothing new for multinomial here)

    -   Works because the internal calculations are for dmultinom are all defined
        for non-integers.
...
-   Larger and more realistic example data set, reparameterizd the vonB (use L2
    rather than t0)
...
$$
\begin{array}{c}{\left[\log L \infty_{i}, \log K_{i}, \log \widetilde{L 2}_{i}\right]^{T} \sim M V N\left(\left[\overline{\log L \infty_{i}}, \overline{\log K_{i}}, \overline{\log \widetilde{L 2}}\right]^{T}, \Sigma\right)} \\L_{i, j} \sim N\left(\operatorname{vonb}\left(\underline{\omega}_{i}, a_{i, j}\right), \sigma_{L, i, j}^{2}\right) \\\sigma_{L, i, j}=\exp \left[\operatorname{int}+\operatorname{slp} * \operatorname{vonb}\left(\underline{\omega}_{i}, a_{i, j}\right)\right] \operatorname{vonb}\left(\underline{\omega}_{i}, a_{i, j}\right)\end{array}
$$
